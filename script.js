...
function momentoAcao() {
  // Pega o valor das sessões no localStorage e coloca na variável sessoes_valor_acao
  let sessoes_valor_acao = localStorage.getItem("sessoes");

  // Verificação se o sessoes_valor_acao é diferente de 1 (Comparação de valores String)
  if (sessoes_valor_acao != "1") {
    document.getElementById("title_sessao").innerHTML =
      sessoes_valor_acao + " sessões restantes";
  } else {
    document.getElementById("title_sessao").innerHTML =
      sessoes_valor_acao + " sessão restante";
  }
  ...
  // Pega o valor de acao do localStorage, já convertendo para Number e adicionando a variável min_acao
  let min_acao = Number(localStorage.getItem("acao"));
  ...
  // Adiciona o valor de min_acao ao h2 que contem a tag minutes_ok
  document.getElementById("minutes_ok").innerHTML = min_acao;
  ...
  function minTimer() {
    ...
    let min_timer = min_timer - 1;
    // Adiciona o valor de min_timer ao h2 que contem a tag minutes_ok
    document.getElementById("minutes_ok").innerHTML = min_timer;
  }
